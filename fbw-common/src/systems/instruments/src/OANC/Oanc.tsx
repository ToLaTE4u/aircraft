...
// Phase 2: Updated for 5 layers instead of 8 (Step 1: merged 0+1, Step 2: merged runways, Step 3 Alt: merged yellow guidance 2+4)
// Layer mapping: 0=Ground(merged), 1=Runways(merged), 2=YellowGuidance(merged), 3=GrayOutlines, 4=BTV
...
export class Oanc<T extends number> extends DisplayComponent<OancProps<T>> {
  ...
  // Phase 2: Reduced from 8 to 5 layers (Step 3 Alt: merged yellow guidance lines)
  ...
  // Phase 2: Reduced from 8 to 5 layers (Step 3 Alt: merged yellow guidance lines 2+4)
  ...
  private layerFeatures: FeatureCollection<Geometry, AmdbProperties>[] = [
    ...
    featureCollection([]), // Layer 2: ALL YELLOW GUIDANCE LINES (taxiway + stand) - MERGED
    featureCollection([]), // Layer 3: TAXIWAY GUIDANCE GRAY OUTLINES (must stay separate)
    featureCollection([]), // Layer 4: DYNAMIC BTV CONTENT (BTV PATH, STOP LINES)
  ];
  ...
  // Phase 2: BTV layer index changed (Step 1: 7→6, Step 2: 6→5, Step 3 Alt: 5→4)
  private readonly btvUtils = new OansBrakeToVacateSelection<T>(
    this.props.bus,
    this.labelManager,
    this.aircraftOnGround,
    this.projectedPpos,
    this.layerCanvasRefs[4],
    this.canvasCentreX,
    this.canvasCentreY,
    this.zoomLevelIndex,
  ...
  private logPerformanceMetrics() {
    ...
    // Phase 2: 5 layers instead of 8 (Step 3 Alt)
